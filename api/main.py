...
@app.post("/search_small/", response_model=List[schemas.ItemResponse])
async def search_items(query: schemas.SearchQuery, db: Session = Depends(get_db)):
    """Search for similar items using vector similarity"""
    try:
        # Generate embedding for search query
        
        query_embedding = get_embedding_small(query.query)
        ...
        results = db.execute(
            text(
                ...
                SELECT
                    ...
                    name,
                    item_data,
                    embedding_openai_small <=> cast(:embedding as vector) as similarity
                FROM items
                ORDER BY embedding_openai_small <=> cast(:embedding as vector)
                LIMIT :limit
                """
            ),
...
@app.post("/search_large/", response_model=List[schemas.ItemResponse])
async def search_items(query: schemas.SearchQuery, db: Session = Depends(get_db)):
    """Search for similar items using vector similarity"""
    try:
        # Generate embedding for search query
        
        query_embedding = get_embedding_large(query.query)
        print(f'search -> query_embedding: {len(query_embedding)}')
        ...
        results = db.execute(
            text(
                ...
                SELECT
                    ...
                    name,
                    item_data,
                    embedding_openai_large <=> cast(:embedding as vector) as similarity
                FROM items
                ORDER BY embedding_openai_large <=> cast(:embedding as vector)
                LIMIT :limit
                """
            ),
            {"embedding": query_embedding, "limit": query.limit},
